...
#### 角色样貌
- ❌ 频繁切换装备 / 变身 / 骑乘切换等导致角色外观或状态改变

#### 自动化采集脚本必须避免的 7 类问题(PDF p9-11)
- ❌ 穿模 + 闪退
- ❌ 原地转圈持续不停
- ❌ 出现闪屏
- ❌ 经常穿模穿树木
...
#### 角色动作(>90% 必须是)
- ✅ 行走 / 跑动 / 视角转动
- ❌ 交互层级超过 2 层(打开背包 → 选道具 → 用道具 = 已 3 层)
- ❌ >10% 时长主体原地静止
- ❌ 单调向前轻微移动
- ❌ NPC 主动开箱子等其他对象驱动场景变化
...
| 网络上行 | 50 Mbps | 200 Mbps |
| 网络延迟 | < 20 ms | — |
| **鼠标 dpi** | **1800** (品牌罗技 / 雷蛇 / 戴尔外星人,必须有 dpi 驱动) | — |
| **系统鼠标指针速度** | **win10 = 6 / win11 = 10**(系统默认值,不要调) | — |

**单台机器月产能估算**: 100-300 clip(取决于场景复杂度 + 操作员熟练度)

### 5.4 游戏复杂度三档分级(PDF p2,影响报价)
| 复杂度 | 单游戏录制总时长 | 单价系数 | 适配场景 |
|---|---|---|---|
| **高 (high)** | 20h+ | 1.0× | 综合性大场景 / 现代都市 / 古代城市 / 复杂生态 |
| **中 (mid)** | 1-20h | 0.7× | 3+ 城市区块 / 野外环境 / 城堡 / 科幻设施 |
| **低 (low)** | < 1h | 0.4× | 1-2 街区 / 废城 / 厂房 / 山洞 / 平原 / 湖泊 |

(单价系数仅供报价参考,具体由双方商务议定。)
...
### 9.4 保密
- 本 PRD 与参考工程**已公开发布**(GitHub repo public + release v0.1.0-rc2),vendor 可自由转发链接
- 但**双方商务信息**不得对外披露:具体单价 / 客户名单 / 月产能上限 / 私下议定的优惠条款
- 操作员真实姓名 / 邮箱 / 手机号 / IP 等 **PII** 永不出现在交付物中
- 与外部讨论本项目时使用代号 **"GameData"**(避免 vendor 客户混淆) 
