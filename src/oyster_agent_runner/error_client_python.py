...
class PIIStripper:
    ...
    PATTERNS = [
        ...
        (re.compile(r'(?:api[_-]?key|apikey|secret[_-]?key|access[_-]?token)\s*[:=]\s*["\']?([a-zA-Z0-9_\-]{16,})["\']?', re.IGNORECASE), r'\1=[REDACTED]'),
...
class ErrorPayload:
    ...
    def __init__(self, error_type: str, error_message: str, traceback_str: str, context: Optional[Dict[str, Any]] = None):
...
class ErrorReporter:
    ...
    def __init__(self, endpoint: str = DEFAULT_ENDPOINT, max_retries: int = MAX_RETRIES, timeout: float = REQUEST_TIMEOUT, max_payload_size: int = MAX_PAYLOAD_SIZE):
    ...
    def report_error(self, error_type: str, error_message: str, traceback_str: str, context: Optional[Dict[str, Any]] = None) -> None:
        ...
        safe_ctx = {k: (PIIStripper.strip(v) if isinstance(v, str) else v) for k, v in (context or {}).items()}
    ...
    def _send(self, json_data: str) -> None:
        ...
        req = urllib.request.Request(self.endpoint, data=data, headers={'Content-Type': 'application/json', 'User-Agent': f'ErrorClient/{__version__}'}, method='POST')
...
def _sys_excepthook(exc_type: type, exc_value: BaseException, tb: Optional[types.TracebackType]) -> None:
    ...
    _get_reporter().report_error(exc_type.__name__, str(exc_value), tb_str, {"source": "sys.excepthook", "thread": threading.current_thread().name})
...
def _threading_excepthook(args: Any) -> None:
    ...
    tb_str = _format_exc(exc_type, exc_value, tb) if tb else ""
    _get_reporter().report_error(exc_type.__name__, str(exc_value), tb_str, {"source": "threading.excepthook", "thread": getattr(args, 'thread_name', 'unknown')})
...
def main(argv: List[str]) -> int:
    ...
    parser = argparse.ArgumentParser(description="Python Error Client - Global error capture and reporting")
    parser.add_argument('--install', action='store_true', help='Install global error handlers')
    parser.add_argument('--endpoint', type=str, default=DEFAULT_ENDPOINT, help=f'Error collection endpoint (default: {DEFAULT_ENDPOINT})')
    parser.add_argument('--version', action='store_true', help='Show version information')
    parser.add_argument('--test', action='store_true', help='Test error reporting by raising a test exception')
