...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--input", "-i", type=Path, required=True, help="Path to vector data file.")
    parser.add_argument(
        "--dim", "-d", type=int, default=3, help="Expected vector dimensionality (default: 3)."
    )
    parser.add_argument(
        "--strict", action="store_true", help="Exit non-zero on any shape mismatch."
    )
    ...
    if violations:
        for idx, actual, row in violations:
            ...
            print(
                f"VIOLATION row {idx}: expected dim={args.dim}, got dim={actual}  "
                f"values={vals}{suffix}",
                file=sys.stderr,
            )
