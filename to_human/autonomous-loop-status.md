...
## Round 626 @ 2026-07-14T11:30:00Z
- Picked: Add test file for bin/red_team_out_of_order_frames.py (red-team JSON frame shuffler). Missing test coverage — 174-line stdlib-only module that locates a frames array in arbitrary JSON, shuffles it to break monotonic frame_id, and writes the result. Public API: find_frames, replace_frames, shuffle_frames, main.
- Result: committed 8f9df266, pushed to fix/prd-test-action-per-second-ruff. Tests pass (34/34), ruff clean. Self-review passed (checked silent error swallow — shuffle_frames surfaces to stderr with exit 1, tests assert on stderr; false-success prevented via concrete ids/contents/return-code assertions; no race conditions (synchronous, tmp_path-scoped); no off-by-one (recursion order pinned via test_prefers_top_dict_frames_key_over_nested_list); security clean (no eval/shell, subprocess uses list form); zero skip/xfail markers, all 34 tests are real assertions; brand isolation clean).
- Justification: PRD gap with clear acceptance — the red-team frame shuffler had zero test coverage and is a critical tool for validating the lint pipeline's monotonic frame_id enforcement (its stated contract).

## Round 627 @ 2026-07-01T08:46:43Z
- Picked: Commit untracked test file tests/bin/test_recorder_audio_loopback.py (with ruff F401 fix for unused `pytest` import). Missing test coverage for WASAPI loopback audio recorder — validates check_wasapi_available (loopback/wasapi/missing/subprocess error/file not found), build_ffmpeg_command (wasapi/dshow-mic modes, duration, codec flags), record_audio (prefer_wasapi, dshow fallback, error return code), main CLI (output/duration/no-wasapi/verbose flags, exit code propagation, output directory creation).
- Result: committed 00d7b946, pushed to fix/prd-test-action-per-second-ruff. Tests pass (26/26), ruff clean (fixed unused `pytest` import). Self-review passed (silent error swallow: both SubprocessError and FileNotFoundError branches explicitly tested with `mock_run.side_effect`; false-success: exact `is True/False` assertions for wasapi boolean, exact cmd list equality via index-based checks, exact returncode propagation in record_audio + main; race conditions: all subprocess calls mocked via `subprocess.run` patch; off-by-one: `cmd.index("-f")` + 1 is safe because `-f` exists in both branches; security: no real subprocess execution, mocks only; skip/xfail: none — test count 26 matches pytest collection; brand isolation: clean). Justification: PRD gap with clear acceptance criteria — bin/recorder_audio_loopback.py is G279/B4 production code for privacy-safe system-audio capture (WASAPI loopback, no microphone) and had zero committed test coverage; the test file was drafted in a prior round but left untracked.
