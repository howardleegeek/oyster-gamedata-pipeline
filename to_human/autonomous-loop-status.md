...
## Round 294 @ 2026-07-04T23:30:00Z
- Picked: Continue in-progress work from interrupted prior tick: surface silent error in bin/real_depth_filler.py — replaced 3 bare `except Exception:` blocks in `select_device`, `_get_torch_dtype`, and `_verify_exr_channel` with `except Exception as e:` + `logger.debug(..., exc_info=True)`. Module-level `logger` was added (import logging + `logger = logging.getLogger(__name__)`). Control flow preserved exactly (still returns "cpu", None, False respectively — fail-soft device/dtype/EXR probes). Added regression test tests/bin/test_real_depth_filler_silent_error.py (7 passed: AST no-bare-except parametrised over the 3 target functions, select_device returns "cpu" + DEBUG log when torch.cuda.is_available raises, _get_torch_dtype returns None + DEBUG log when torch import is blocked, _verify_exr_channel returns False + DEBUG log when OpenEXR import is blocked, module-level logger is wired up with name "real_depth_filler"). py_compile clean on both files; ruff clean on both files. Self-review: silent-swallow fixed in all 3 helpers, control flow preserved (return values unchanged, fallback behavior unchanged for device/dtype/EXR), no race conditions (synchronous in-process probes), no off-by-one, no new attack surface (broad except is intentional for fail-soft device/dtype/EXR probes — `# noqa: BLE001` marks intent), 7 distinct test assertions none masked as passing, no skip/xfail. Result: tests pass, lint clean, ready to commit.
- Result: committed bd988c4d, pushed to origin/main

## Round 295 @ 2026-07-04T19:49:06Z
- Picked: Surface silent error in bin/spectator_follow.py RconClient.authenticate() trailing-receive best-effort drain — replaced bare `except Exception: pass` with `except Exception as e:` + `logger.debug(..., exc_info=True)`. Added module-level `logger = logging.getLogger(__name__)`. Control flow UNCHANGED: auth verdict still based on the first response packet via `response_id == packet_id`. Added regression test: tests/bin/test_spectator_follow_silent_error.py (5 passed: AST no-bare-except in authenticate, module logger attached to bin.spectator_follow not root, trailing-recv failure no longer propagates + auth still succeeds, DEBUG log emitted with exception text, happy path still works). py_compile clean; ruff clean. Self-review: silent-swallow fixed, control flow preserved (verdict is from first response, not the best-effort trailing recv), no race/sync (single-threaded client method), no off-by-one (recv sequence unchanged), no new attack surface (module-namespaced debug log), 5 distinct assertions across 5 tests none masked as passing.
- Result: committed 464101de, pushed to origin/main

## Round 296 @ 2026-07-05T00:00:00Z
- Picked: Surface silent errors in bin/upload_status.py get_status() — replaced 2 bare `except Exception: pass` blocks (around `datetime.fromisoformat()` for completed_at and created_at) with `except Exception as e:` + `logger.debug(..., exc_info=True)`. Added module-level `logger = logging.getLogger(__name__)`. Control flow unchanged (corrupt timestamps still skipped silently from `last_24h` counters). Regression test: tests/bin/test_upload_status_silent_error.py (5 passed: AST no-bare-except in get_status, module-level logger present, ≥2 logger.debug calls in except branches, runtime corrupt-timestamp does not crash + emits debug log, valid recent timestamp still counted + old failed timestamp still excluded). py_compile clean; ruff clean; git add both files; committed 0cc7709b and pushed to origin/main.
- Result: committed 0cc7709b, pushed to origin/main
