...
## Round 379 @ 2026-06-26T13:30:00Z

- Picked: ruff format bin/recorder_depth_filler.py (smallest unformatted bin file: 272 lines; line-wrap ffmpeg arg list (`-i`, `-vf`, `-vsync`), wrap return-arg tuple in fill_clip_depth, wrap logger.info args, multi-line argparse add_argument for --fps and --expected-frames). Justification: measurable code smell (ruff format --check fail), single-file scope, no behavior change (purely cosmetic wrapping), no test references to formatter changes, ruff check clean before+after, import smoke + py_compile + 7/7 tests pass, follows established cadence of small-bin-file formats (Rounds 254-378).
- Result: committed d5e75215 (ruff format applied: line-wrap ffmpeg arg list, return args, argparse add_argument, logger.info args; 1 file changed, 26 insertions(+), 14 deletions(-); ruff check + ruff format --check clean; py_compile + import smoke + 7/7 tests in tests/bin/test_recorder_depth_filler.py pass; pushed to origin/fix/prd-test-action-per-second-ruff. Self-review: cosmetic reformat only — no silent error swallow (RuntimeError ffmpeg-failed message preserved character-for-character), no race, no off-by-one, no security impact, no test masking (no skip/xfail), no brand cross-reference, no module-level side effect, no ffmpeg argv semantics change, no argparse semantics change.)
