...
class SegmentationProvider:
    ...
    def process_frame(
        self, frame: Any, frame_id: Optional[int] = None, timestamp: Optional[float] = None
    ) -> Result:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    config = Config(
        ...
        enable_tracking=args.track and not args.no_track,
        format=args.format,
    )
    ...
    if args.single:
        ...
        output_path = (
            args.output
            if args.output.endswith(f".{args.format}")
