...
class DRFailoverValidator:
    ...
    def verify_failover_ingest(self) -> Tuple[bool, str]:
        ...
        if self.dry_run:
            time.sleep(2)
            self._add_result(
                "failover_ingest", "SUCCESS", "Dry run - would check ingest", {"url": ingest_url}
            )
            return True, "Dry run"
        ...
        while time.time() < deadline:
            ...
            if ok:
                ...
                self._add_result(
                    "failover_ingest",
                    "SUCCESS",
                    f"Ingest available in {elapsed:.1f}s",
                    {"url": ingest_url},
                )
                return True, f"Available in {elapsed:.1f}s"
            time.sleep(self.CHECK_INTERVAL)
        self._add_result(
            "failover_ingest", "FAILURE", f"Timeout after {self.timeout}s", {"url": ingest_url}
        )
        ...
        try:
            ...
            if details["files_found"] > 0:
                self._add_result(
                    "postgres_backup",
                    "SUCCESS",
                    f"Found {details['files_found']} backup files",
                    details,
                )
    ...
    def run_all_checks(self) -> Dict[str, Any]:
        ...
        self.results["overall_status"] = (
            "SUCCESS" if not failed else ("PARTIAL" if len(failed) < len(checks) else "FAILURE")
        )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--timeout", type=int, default=60, help="Max wait for failover (default: 60s)"
    )
    ...
    validator = DRFailoverValidator(
        ...
        failover_url=args.failover_url,
    ...
    return (
        ...
        else (1 if results["overall_status"] == "PARTIAL" else 2)
    )
