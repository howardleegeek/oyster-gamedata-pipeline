...
def _body_has_nan_or_inf_residual(node: ast.FunctionDef) -> bool:
    ...
    for sub in ast.walk(node):
        ...
        for inner in ast.walk(sub):
            if isinstance(inner, ast.Attribute) and isinstance(inner.value, ast.Name) and inner.value.id == "math" and inner.attr in {"nan", "inf"}:
                return True
            if isinstance(inner, ast.Call):
                fn = inner.func
                if isinstance(fn, ast.Name) and fn.id == "float" and inner.args and isinstance(inner.args[0], ast.Constant):
