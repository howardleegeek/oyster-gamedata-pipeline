...
class Config:
    ...
    def __init__(
        ...
        self.state_file = state_file or Path(
            os.path.expanduser("~"), ".secret_rotator", "state.json"
        )
    ...
    def from_yaml(cls, path: Path, **kw: Any) -> "Config":
        ...
        return cls(
            ...
            iam_user=data.get("iam_user", "secret-rotator-user"),
            **kw,
        )
...
class RotationState:
    ...
    def promote_secondary(self) -> None:
        ...
        self.state.pop("secondary_created_at", None)
        self.state.pop("primary_deactivated_at", None)
...
class IAMRotator:
    ...
    def rotate(self, state: RotationState) -> None:
        ...
        if not state.secondary_key_id:
            if len(keys) >= 2:
                log.error("Already 2 keys.")
                sys.exit(1)
            if self.cfg.dry_run:
                log.info("[DRY-RUN] Would create key.")
                return
            nk = self.create_access_key()
            state.set_secondary(nk["AccessKeyId"])
            state.save()
            log.info("Created secondary %s", nk["AccessKeyId"])
            return
        if state.secondary_created:
            end = state.secondary_created + datetime.timedelta(days=self.cfg.overlap_days)
            if now < end:
                log.info("Overlap active; %s remaining.", end - now)
                return
        if not state.primary_deactivated_at and state.active_key_id:
            if self.cfg.dry_run:
                log.info("[DRY-RUN] Would deactivate %s", state.active_key_id)
                return
            self.deactivate_key(state.active_key_id)
            state.deactivate_primary()
            state.save()
            log.info("Deactivated %s", state.active_key_id)
            return
        del_after = (
            (state.primary_deactivated_at + datetime.timedelta(days=self.cfg.overlap_days))
            if state.primary_deactivated_at
            else None
        )
        if del_after and now >= del_after:
            if self.cfg.dry_run:
                log.info("[DRY-RUN] Would delete & promote.")
                return
            if state.active_key_id:
                self.delete_key(state.active_key_id)
            state.promote_secondary()
            state.save()
            log.info("Promoted secondary; old key deleted.")
            return
    ...
    def status(self, state: RotationState) -> None:
        ...
        for k in keys:
            print(f"  {k['AccessKeyId']}  {k['Status']}  created={k.get('CreateDate')}")
        print(f"Active   : {state.active_key_id or '(none)'}")
        print(f"Secondary: {state.secondary_key_id or '(none)'}")
        if state.secondary_created:
            print(f"Secondary created: {state.secondary_created}")
        if state.primary_deactivated_at:
            print(f"Primary deactivated: {state.primary_deactivated_at}")
    ...
    def init_state(self, state: RotationState) -> None:
        ...
        state.state["active_key_id"] = active["AccessKeyId"]
        state.save()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        prog="secret_rotator", description="90-day automatic IAM key rotation with overlap window."
    )
    p.add_argument("--config", type=Path)
    p.add_argument("--iam-user", default=None)
    p.add_argument("--region", default=None)
    p.add_argument("--rotation-days", type=int, default=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.config:
        cfg = Config.from_yaml(args.config, dry_run=args.dry_run, state_file=args.state_file)
    if args.iam_user:
        cfg.iam_user = args.iam_user
    if args.region:
        cfg.aws_region = args.region
    if args.rotation_days is not None:
        cfg.rotation_days = args.rotation_days
    if args.overlap_days is not None:
        cfg.overlap_days = args.overlap_days
    state = RotationState(cfg.state_file)
    rotator = IAMRotator(cfg)
    if args.command == "rotate":
        rotator.rotate(state)
    elif args.command == "status":
        rotator.status(state)
    elif args.command == "init":
        rotator.init_state(state)
    else:
