...
def spectate_loop(
    ...
    logging.info(
        f"Interval: {interval_sec}s, Duration: {'infinite' if duration_sec is None else f'{duration_sec}s'}"
    )
    ...
    try:
        while not stop_requested:
            ...
            if not success:
                ...
                logging.error(
                    f"Failed to send spectate command (consecutive failures: {consecutive_failures})"
                )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--interval", type=float, default=5.0, help="Interval between spectate commands (seconds)"
    )
    parser.add_argument(
        "--duration",
        type=float,
