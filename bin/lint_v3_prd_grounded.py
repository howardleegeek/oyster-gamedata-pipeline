#!/usr/bin/env python3
"""
G165 · bin/lint_v3_prd_grounded.py — Stream BC rc17.1 (Audit E rewrite)

Cluster A: Full PRD page-by-page lint tool. 33 acceptance criteria (including #38 audio continuity).
...
class LintReport:
    ...
    total_checks: int = 33  # rc17.1: bumped from 27 to 32 (Stream BC adds 28-32), plus #38 audio continuity
    passed_count: int = 0
    failed_count: int = 0
...
def _check_audio_continuity(d: Path, rpt: LintReport) -> None:
    ...
    rpt.add(LintResult(
        ...
        "Audio continuity check passed - no silent gaps > 2s",
        {
            ...
            "analysis_summary": audio_check.get("analysis", {}).get("dbfs_summary", {})
        }
    ))
...
def run_all_checks(data_dir: Path) -> LintReport:
    """Run all 33 lint checks on the data directory."""
    rpt = LintReport(data_dir=data_dir, total_checks=33)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description=("G165 PRD Grounded Lint Tool — rc17.1 (32 criteria, "
                     "Audit E rewrite)"))
