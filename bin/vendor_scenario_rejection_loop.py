...
def format_rejection_email(vendor_email: str, metadata: ClipMetadata,
                           errors: list[str]) -> str:
...
def simulate_rejection_loop(clip_path: Path, metadata: ClipMetadata,
                           vendor_email: str, output_dir: Optional[Path] = None) -> dict:
    ...
    result = {
        "clip_id": metadata.clip_id, "vendor_id": metadata.vendor_id,
        "validation": {"is_valid": validation.is_valid,
                      "errors": validation.errors, "warnings": validation.warnings},
        "email_sent": False, "output_dir": str(output_dir)
    }
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    metadata = ClipMetadata(
        vendor_id=args.vendor_id, clip_id=args.clip_id, filename=clip_path.name,
        file_size_bytes=clip_path.stat().st_size if clip_path.exists() else 0,
        format=args.format, duration_seconds=args.duration, resolution=args.resolution
    )
