...
def p1_trajectory_smoothness(game_state: list) -> dict:
    ...
    return {
        ...
        "verdict": (
            "smooth"
            if rho1 > 0.5
            else "jittery" if rho1 > 0.1 else "DISCONTINUOUS (teleport / fake?)"
        ),
    }
...
def p2_mouse_camera_coherence(inputs: list, action_camera: list, game_state: list = None) -> dict:
    ...
    return {
        ...
        "verdict": (
            "STRONG (mouse drives camera)"
            if abs(result["peak_corr"]) > 0.4
            else (
                "weak (possible decoupling)"
                if abs(result["peak_corr"]) > 0.15
    ...
    return {
        ...
        "verdict": (
            ...
            if deep_pct >= 95
                ...
                f"Y_UP probable but some sign-flip ({deep_pct:.0f}% — investigate)"
                if deep_pct >= 70
    ...
    return {
        ...
        "verdict": (
            ...
            if 4.0 < max_speed < 7.5
                ...
                "m/s but slow (walking only, no sprint)"
    ...
    return {
        ...
        "verdict": (
            ...
            if 0.3 <= cv <= 2.0
                ...
                "ROBOTIC (CV too low — fixed-rate bot?)"
                if cv < 0.3
