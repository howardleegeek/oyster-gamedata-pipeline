...
def determine_verdict(flags: Dict[str, Any]) -> tuple[str, List[str]]:
    ...
    if flags['player_username']:
        recommendations.append(f"Player username '{flags['player_username']}' should be pseudonymized if shipped to buyer")
    ...
    if flags['credit_cards']:
        ...
        recommendations.append(f"Credit card numbers found: {len(flags['credit_cards'])} - IMMEDIATE ACTION REQUIRED")
