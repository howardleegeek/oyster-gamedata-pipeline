...
class UploadDaemon:
    ...
    def __init__(self, watch_dir: Path = DEFAULT_WATCH_DIR, max_kbps: int = DEFAULT_MAX_KBPS,
                 s3_endpoint: str = "http://localhost:8080", dry_run: bool = False):
        ...
        for session_id, session in self.state.get("sessions", {}).items():
            if isinstance(session, UploadSession):
                ...
                if session_dict.get("chunks"):
                    session_dict["chunks"] = [
                        asdict(c) if isinstance(c, ChunkInfo) else c 
                        for c in session_dict["chunks"]
                    ]
    ...
    def _complete_multipart_upload(self, session_id: str, upload_id: str, parts: List[Dict]) -> Dict:
    ...
    def _process_session(self, file_path: Path) -> bool:
        ...
        for chunk in session.chunks:
            ...
            try:
                ...
                logger.info(f"Uploaded chunk {chunk.index}/{len(session.chunks)} "
...
def main():
    ...
    parser.add_argument("--max-kbps", type=int, default=DEFAULT_MAX_KBPS,
                       help="Maximum bandwidth in KB/s (default: 5000)")
    parser.add_argument("--s3-endpoint", type=str, default="http://localhost:8080",
                       help="S3 API endpoint URL")
    parser.add_argument("--interval", type=int, default=60,
                       help="Scan interval in seconds (default: 60)")
    parser.add_argument("--dry-run", action="store_true",
                       help="Don't actually upload or rename files")
