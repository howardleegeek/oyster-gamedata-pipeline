...
def extract_voxel_window(
    world_dir: str, player_pos: Tuple[float, float, float], frame_index: int
    ...
    try:
        centre_x, centre_y, centre_z = (
            int(round(player_pos[0])),
            int(round(player_pos[1])),
            int(round(player_pos[2])),
        )
...
def capture_frame(
    ...
    player_pos = (
        player_pos_override if player_pos_override else load_player_position(world_dir, frame_index)
    )
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="inventory_voxel_capture",
        description="Per-frame inventory + 3x3x3 voxel-window block-ID capture.",
    )
