...
def write_output(routes: List[Dict[str, Any]], output_format: str, output_path: str) -> None:
    ...
    if output_format == 'json':
        with open(output_path, 'w') as f:
            json.dump({
                'routes': routes,
                'statistics': calculate_statistics(routes)
            }, f, indent=2)
    
    elif output_format == 'csv':
        with open(output_path, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=['route_id', 'route_type', 'wasd_key', 'description'])
            writer.writeheader()
            for route in routes:
...
def main(argv: List[str]) -> int:
    ...
    try:
        ...
        if args.verify:
            ...
            type1_percent = (stats['route_type_distribution'].get(1, 0) / stats['total_routes']) * 100
            type2_percent = (stats['route_type_distribution'].get(2, 0) / stats['total_routes']) * 100
