...
def load_sample_data(vendor_id: str, date: str) -> dict[str, Any]:
    """
    Load sample metrics for a vendor on a given date.
    
    In production, this would read from a database or API.
    For alpha, we generate realistic sample data.
    ...
    # Generate deterministic sample data based on vendor_id hash
    vendor_hash = hash(vendor_id) % 1000
    base_metrics["orders"] = 50 + (vendor_hash % 200)
    base_metrics["revenue"] = float(500 + (vendor_hash * 3) % 5000)
    base_metrics["customers"] = 30 + (vendor_hash % 150)
    base_metrics["active_products"] = 10 + (vendor_hash % 50)
    
    if base_metrics["orders"] > 0:
        base_metrics["avg_order_value"] = base_metrics["revenue"] / base_metrics["orders"]
    
    base_metrics["return_rate"] = 0.02 + (vendor_hash % 10) * 0.005
...
"""
    ...
    parser.add_argument(
        "--vendor", "-v",
        required=True,
        help="Vendor ID to display metrics for"
    )
    parser.add_argument(
        "--date", "-d",
    ...
    parser.add_argument(
        "--output", "-o",
        default=None,
        help="Path to write dashboard output"
    )
    parser.add_argument(
        "--quiet", "-q",
        action="store_true",
    ...
    # Load metrics
    try:
        metrics = load_sample_data(args.vendor, args.date)
