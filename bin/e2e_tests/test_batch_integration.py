...
def create_test_batch(temp_dir: Path) -> str:
    ...
    batch_manifest = {"batch_id": "test_batch_e2e", "quota": TEST_QUOTA, "routes": []}
...
def run_route_planner(session_dir: str, quota: Dict[int, int]) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        batch_manifest = {"batch_id": "test_batch_e2e", "quota": quota, "routes": []}
        ...
        try:
            ...
            if manifest_path.exists():
                ...
                return {"status": "FAIL", "evidence": "batch_manifest.json not found after run"}

            return {"status": "PASS", "manifest": updated_manifest}
...
def validate_route_selection(manifest: Dict[str, Any], quota: Dict[int, int]) -> Dict[str, Any]:
    ...
    if route_type == expected_route_type:
        return {"status": "PASS", "evidence": f"route_planner picked route_type={route_type}"}
    else:
        return {
            "status": "FAIL",
...
def main():
    ...
    parser.add_argument(
        "--quota", type=str, default=None, help="JSON quota config (default: {1:3,2:5,3:2})"
    )
