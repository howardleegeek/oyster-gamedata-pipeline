...
def write_audit_entry(
    audit_path: Path, host: str, port: int, obs_key_hash: str, result: dict, attempt_id: str
) -> None:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    logger.info(
        "Red-team attempt %s — target %s:%d (key hash: %s…)",
        attempt_id,
        args.host,
        args.port,
        obs_key_hash[:12],
    ...
    result = attempt_auth(args.host, args.port, args.obs_key, args.tls, args.timeout, args.path)
