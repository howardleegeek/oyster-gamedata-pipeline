...
def analyze_clip(clip_data: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    if camera_data:
        ...
        combined_var = math.sqrt(
            _variance(x_vals) ** 2 + _variance(y_vals) ** 2 + _variance(z_vals) ** 2
        )
...
def run_detection(
    input_path: Path, config: dict[str, Any], output_path: Optional[Path] = None
) -> dict[str, Any]:
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--farming-n-clips", type=int, default=3, help="Clips to flag as farming (default: 3)"
    )
            ...
            if results["anomalies"]:
                ...
                for anomaly in results["anomalies"]:
                    if "clip_id" in anomaly:
                        ...
                        print(
                            f"  - Farming: {anomaly.get('count')} clips with identical trajectory"
                        )

    return (
        1
        if (results["clips_with_anomalies"] > 0 or results["farming_patterns_detected"] > 0)
        else 0
    )
