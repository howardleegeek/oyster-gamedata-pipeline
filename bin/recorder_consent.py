...
def run_oauth_flow(provider: str = "google") -> dict:
    ...
    callback_thread = threading.Thread(target=lambda: wait_for_callback(port))
...
def main():
    ...
    parser.add_argument(
        "--provider", choices=["google", "discord"], default="google", help="OAuth provider to use"
    )
    parser.add_argument(
        "--skip-browser", action="store_true", help="Don't open browser (print URL instead)"
    )
    ...
    try:
        ...
        consent_record = create_consent_record(
            user_id=payload.get("sub"), provider=payload.get("oauth_provider", args.provider)
        )
