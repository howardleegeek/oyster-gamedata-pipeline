...
def get_git_log_summary(since: str = "1 day ago") -> list[dict]:
    ...
    for line in result.stdout.strip().split("\n"):
        ...
        if len(parts) == 4:
            ...
            commits.append({"sha": sha, "author": author, "message": message, "date": date})
...
def build_dashboard(
    ...
    lines.append(
        f"- **Files changed**: {files_changed} (added: {files_changed // 2}, modified: {files_changed // 4})"
    )
    ...
    if commits:
        ...
        for commit in commits[:10]:  # Show last 10 commits
            ...
            lines.append(
                f"| {time_str} | {sha_short} | {commit['author']} | {commit['message'][:50]} |"
            )
    else:
    ...
    for dir_name in sorted_dirs:
        ...
        lines.append(
            f"| {display_name} | {counts['py']} | {counts['sh']} | {counts['md']} | {counts['total']} |"
        )
...
Examples:
  ...
  %(prog)s --sprint-name "Production push 2026-05" --output dashboard.md
    ...
    parser.add_argument(
        "--since", default="1 day ago", help="Git log since parameter (default: '1 day ago')"
    )
    ...
    dashboard = build_dashboard(
        repo_root=args.repo_root, since=args.since, sprint_name=args.sprint_name
    )
