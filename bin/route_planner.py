...
def pick_next_route_type(scene: str, manifest: Dict[str, Any]) -> tuple:
    ...
    if current < target:
        reason = (
            f"scene quota: route_type={selected_route_type} has only {current} of {target} needed"
        )
    else:
...
def main():
    ...
    parser.add_argument("--scene", required=True, help="Scene name (e.g., Overworld_NewWorld)")
    parser.add_argument(
        "--batch-id", required=True, help="Batch identifier (e.g., 2026-05-batch-1)"
    )
    parser.add_argument("--operator-id", default="", help="Operator identifier")
    parser.add_argument(
        "--record-session", action="store_true", help="Record this assignment in the manifest"
    )
    parser.add_argument(
        "--session-id", default="", help="Session ID to record (used with --record-session)"
    )
