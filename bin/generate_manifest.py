...
def extract_clip_metadata(tarball_path: str) -> dict:
    ...
    with tarfile.open(tarball_path, "r:gz") as tar:
        ...
        # Count depth EXRs
        depth_count = sum(
            1 for m in members if m.name.endswith(".exr") and "depth" in m.name.lower()
        )
...
def build_manifest(
    ...
    for i, tarball in enumerate(tarballs):
        ...
        if op_id not in operator_stats:
            operator_stats[op_id] = {
                "clip_count": 0,
                "routes": {"normal": 0, "special": 0, "loop": 0},
            }
    ...
    for op_id, stats in sorted(operator_stats.items()):
        operators.append(
            {"operator_id": op_id, "clip_count": stats["clip_count"], "routes": stats["routes"]}
        )
...
def validate_manifest(manifest_path: str) -> tuple[bool, list[str]]:
    ...
    if "manifest_sha256" in manifest:
        ...
        if stored_hash != computed_hash:
            errors.append(
                f"Manifest SHA256 mismatch: stored={stored_hash[:16]}..., computed={computed_hash[:16]}..."
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.batch_dir or not args.vendor_id or not args.batch_id:
        parser.error(
            "--batch-dir, --vendor-id, and --batch-id are required for manifest generation"
        )
