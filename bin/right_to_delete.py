...
def find_sessions_for_contributor(
    contributor_id_hash: str, sessions_dir: Path = Path(".")
) -> List[Path]:
    ...
    for session_dir in sessions_dir.iterdir():
        ...
        if session_file.exists():
            try:
                ...
                contrib_id = session_data.get("contributor_id_hash") or session_data.get(
                    "contributor_id"
                )
                if contrib_id == contributor_id_hash:
...
def mark_for_deletion(
    contributor_id: str, requested_at: str, reason: str, sessions_dir: Path = Path(".")
) -> Dict[str, Any]:
    ...
    requested_date = (
        ...
        else datetime.strptime(requested_at, "%Y-%m-%d")
    )
...
def process_deletions(
    sessions_dir: Path = Path("."), dry_run: bool = False
) -> List[Dict[str, Any]]:
...
Examples:
  ...
  oyster-delete --list
    ...
    parser.add_argument(
        "--requested-at", type=str, default=None, help="Date of deletion request (YYYY-MM-DD)"
    )
    ...
    if args.contributor_id and args.requested_at:
        result = mark_for_deletion(
            args.contributor_id, args.requested_at, args.reason, args.sessions_dir
        )
