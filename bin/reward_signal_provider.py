    ...
    def generate_trajectory(
        ...
        for i, progress in enumerate(progress_list):
            ...
            reward, done, _ = self.step(
                ...
                is_success=success_at_end and is_terminal,
            )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--episode-length", "-n", type=int, default=1000, help="Max steps per episode"
    )
    parser.add_argument(
        "--success-threshold", "-t", type=float, default=0.95, help="Success threshold"
    )
    parser.add_argument("--output", "-o", type=Path, help="Output file for episode summary (JSONL)")
    parser.add_argument(
        "--demo", action="store_true", help="Run demonstration with simulated trajectory"
    )
