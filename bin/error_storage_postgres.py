...
def _build_model():
    ...
    class Error(Base):  # type: ignore[valid-type]
        ...
        def __repr__(self) -> str:
            return (
                f"<Error(id={self.id}, severity={self.severity!r}, "
...
def compute_dedup_key(error_class: str, traceback_text: str, context: Optional[Dict] = None) -> str:
    """SHA-256 hex digest for deduplication."""
    payload = json.dumps(
        {"error_class": error_class, "traceback_text": traceback_text, "context": context or {}},
        sort_keys=True,
        default=str,
    )
...
def purge_old_errors(db_url: str, retention_days: int = DEFAULT_RETENTION_DAYS) -> int:
...
def get_stats(db_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        sev = {
            s: c
            for s, c in sess.query(Error.severity, sa.func.count(Error.id))
            .group_by(Error.severity)
            .all()
        }
        src = {
            s: c
            for s, c in sess.query(Error.source, sa.func.count(Error.id))
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="error_storage_postgres", description="PostgreSQL error storage — Alembic + CRUD CLI."
    )
...
def _cmd_insert(a: argparse.Namespace) -> int:
    rid = insert_error(
        a.db_url, a.severity, a.source, a.error_class, a.traceback, a.user_id, json.loads(a.context)
    )
...
def _cmd_query(a: argparse.Namespace) -> int:
    rows = query_errors(a.db_url, a.severity, a.source, a.user_id, a.dedup_key, a.limit, a.offset)
