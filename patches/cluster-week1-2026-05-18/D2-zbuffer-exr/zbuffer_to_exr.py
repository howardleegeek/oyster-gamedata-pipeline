...
def find_nearest_tick(
    timestamp_ns: int, tick_timestamps: Dict[int, int], max_gap_ms: int
...
def zbuffer_to_exr(session_dir: Path, max_gap_ms: int = 50, fallback_on_miss: bool = True) -> bool:
    ...
    for frame_id, timestamp_ns in camera_frames:
        ...
        if result is None:
            ...
            if fallback_on_miss:
                logger.debug(
                    f"Frame {frame_id}: no tick within {max_gap_ms}ms, skipping (will use DA-V2 fallback)"
                )
            continue
...
def main():
    ...
    parser.add_argument(
        "--max-gap-ms", type=int, default=50, help="Maximum alignment gap in milliseconds"
    )
    parser.add_argument(
        "--fallback-on-miss",
        type=str,
        default="true",
        choices=["true", "false"],
        help="Whether to fallback to DA-V2 for missed frames",
    )
    ...
    success = zbuffer_to_exr(
        Path(args.session_dir), max_gap_ms=args.max_gap_ms, fallback_on_miss=fallback_on_miss
    )
