...
def process_session(session_dir: Path) -> Tuple[str, List[Dict[str, Any]], int, int]:
    ...
    for root, dirs, files in os.walk(session_dir):
        for file in files:
            ...
            file_list.append({"path": str(rel_path), "sha256": file_hash, "bytes": file_size})
...
def main():
    ...
    parser.add_argument("session_dirs", nargs="+", help="Session directories to bundle")
    parser.add_argument(
        "--output-dir", required=True, help="Output directory for tarball and manifest"
    )
    ...
    for session_dir in session_paths:
        session_id, file_list, file_count, session_bytes, session_sha256 = process_session(
            session_dir
        )
        ...
        print(
            f"  {session_id}: {file_count} files, {session_bytes / 1024 / 1024:.1f} MB, sha256: {session_sha256[:8]}..."
        )
