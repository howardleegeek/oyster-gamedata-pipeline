...
def test_process_session():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Process session
        session_id, file_list, file_count, total_bytes, session_sha256 = process_session(
            session_dir
        )
        ...
        concat_hashes = "".join(
            sorted(hashlib.sha256(content).hexdigest() for content in files.values())
        )
...
def test_cli_help():
    ...
    result = subprocess.run(
        [sys.executable, "bin/batch_bundler.py", "--help"], capture_output=True, text=True
    )
    assert result.returncode == 0
