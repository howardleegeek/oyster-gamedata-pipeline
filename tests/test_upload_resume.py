...
from bin.upload_daemon import (
    UploadDaemon, UploadSession, ChunkInfo, UploadState,
    CHUNK_SIZE, STATE_FILE
)
...
class TestUploadResume(unittest.TestCase):
    ...
    def test_idempotent_upload(self):
        ...
        if session.session_id in daemon.state["sessions"]:
            ...
            if isinstance(existing, UploadSession) and existing.state == UploadState.COMPLETED.value:
                # Should skip
