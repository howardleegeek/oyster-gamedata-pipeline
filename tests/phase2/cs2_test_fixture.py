...
def make_synthetic_ticks_df(n: int = 100) -> dict:
    ...
    return {
        ...
        "m_vecVelocity": [[rng.uniform(-500, 500) for _ in range(3)] for _ in range(n)],
    }
...
def _unit_quat(rng: random.Random) -> list[float]:
    ...
    return [
        s1 * math.sin(6.2832 * u2),
        s1 * math.cos(6.2832 * u2),
        s2 * math.sin(6.2832 * u3),
        s2 * math.cos(6.2832 * u3),
    ]
...
def assert_buyer_frames_well_formed(frames: list[dict]) -> None:
    ...
    for i, f in enumerate(frames):
        ...
        for field, length in [("position", 3), ("rotation", 4), ("oula", 3), ("velocity", 3)]:
            v = f[field]
            assert isinstance(v, (list, tuple)), f"Frame {i}: {field} not list"
            assert len(v) == length, f"Frame {i}: {field} len {len(v)} != {length}"
            assert all(isinstance(x, (int, float)) for x in v), f"Frame {i}: {field} non-numeric"
        norm = math.sqrt(sum(x * x for x in f["rotation"]))
        assert abs(norm - 1.0) < 1e-6, f"Frame {i}: quat norm {norm}"
        assert all(-180.0 <= x <= 180.0 for x in f["oula"]), f"Frame {i}: oula out of range"
