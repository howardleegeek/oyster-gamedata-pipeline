...
def find_patch_file():
    ...
    diff_paths = [
        ...
        pathlib.Path("/private/tmp/cluster-2026-05-18-mod-build/patches/depth_zbuffer_capture.diff"),
        pathlib.Path("/private/tmp/cluster-2026-05-18-integration/integrated/patches/depth_zbuffer_capture.diff"),
    ]
...
def test_patch_applies_to_mod_stub():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        subprocess.run(["git", "config", "user.email", "test@test.com"], cwd=mod_dir, check=True, capture_output=True)
        subprocess.run(["git", "config", "user.name", "Test"], cwd=mod_dir, check=True, capture_output=True)
        ...
        subprocess.run(["git", "commit", "-m", "initial"], cwd=mod_dir, check=True, capture_output=True)
...
def find_zbuffer_to_exr_script():
    ...
    script_paths = [
        ...
        pathlib.Path("/private/tmp/cluster-2026-05-18-integration/integrated/bin/zbuffer_to_exr.py"),
    ]
