    ...
    def test_timeout_minutes(self, workflow):
        ...
        for job_name, job_def in jobs.items():
            ...
            assert timeout is not None, (
                f"Job '{job_name}' must have timeout-minutes"
            )
            assert timeout <= 5, (
                f"Job '{job_name}' timeout must be ≤ 5 min, got {timeout}"
            )
...
class TestSteps:
            ...
            if "rust-toolchain" in uses:
                ...
                if version:
                    assert version == "stable", (
                        f"Rust version should be 'stable', got '{version}'"
                    )

    def test_has_cache_step(self):
        uses_steps = [s.get("uses", "") for s in self.steps]
        assert any("actions/cache" in u for u in uses_steps), (
            "Must have a cache step"
        )
    ...
    def test_cache_includes_cargo_home(self):
        ...
        assert ".cargo" in cache_path, (
            f"Cache path must include ~/.cargo, got: {cache_path}"
        )
    ...
    def test_cargo_check_present(self):
        run_steps = [s for s in self.steps if "run" in s]
        combined = " ".join(s["run"] for s in run_steps)
        assert "cargo check" in combined, (
            "Must run 'cargo check'"
        )
    ...
    def test_no_cargo_build(self):
        ...
        assert "cargo build" not in combined, (
            "Must not run 'cargo build' — only 'cargo check'"
        )
    ...
    def test_no_cargo_test(self):
        ...
        run_steps = [s for s in self.steps if "run" in s]
        combined = " ".join(s["run"] for s in run_steps)
        assert "cargo test" not in combined, (
            "Must not run 'cargo test'"
        )
    ...
    def test_cargo_check_flags(self):
        ...
        assert "--release" in combined, (
            "cargo check should use --release flag"
        )
    ...
    def test_pr_comment_on_failure(self):
        ...
        failure_steps = [
            s for s in self.steps
            if s.get("if", "").startswith("failure()")
        ]
        assert len(failure_steps) > 0, (
            "Must have a step with 'if: failure()' for PR comment"
        )
...
class TestGrepChecks:
    def test_grep_cargo_check_in_workflow(self):
        ...
        assert "cargo check" in content, (
            "grep 'cargo check' must match in workflow file"
        )

    def test_grep_vendor_recorder_in_paths(self):
        with open(WORKFLOW_PATH, "r") as fh:
            content = fh.read()
        assert "vendor/recorder" in content, (
            "grep 'vendor/recorder' must match in workflow file"
        )
