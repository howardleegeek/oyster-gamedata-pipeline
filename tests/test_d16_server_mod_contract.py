...
def test_server_environment_is_star_or_dedicated():
    ...
    assert env in ("*", "server"), f"environment={env!r} prevents server-side load"
...
def test_server_session_dir_path_convention():
    ...
    src = (
        ...
        / "recorder"
        / "server"
        / "ServerSessionDir.java"
    ).read_text()
    assert "oyster_state" in src
    assert ".jsonl" in src
    # Should NOT use the client's user.home/Documents path
    assert "user.home" not in src, "server-side path must be server-relative, not user-home"
...
def test_pipeline_shell_discovers_jsonl():
    ...
    assert "oyster_state" in src
    assert "${BOT_USERNAME}" in src, "pipeline must look up JSONL by bot username"
