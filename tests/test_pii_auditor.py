...
from pii_auditor import (
    scan_file_for_pii, scan_session, determine_verdict, audit_session,
    is_private_ip, luhn_check, PATTERNS
)
from pii_redactor import (
    pseudonymize_username, mask_ip, redact_file_content,
    redact_session, sha8
)
from right_to_delete import (
    hash_contributor_id, mark_for_deletion, check_deletion_status,
    ensure_oyster_dir, DELETION_LOG
)
...
class TestIntegration:
    ...
    def test_full_audit_and_redact_workflow(self, tmp_path):
        ...
        with open(inputs_file, 'w') as f:
            f.write(json.dumps({
                "chat": "Hello John Smith, my email is john@test.com"
            }) + "\n")
