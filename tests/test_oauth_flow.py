...
class TestGoogleOAuth:
    ...
    def test_google_login_with_redirect_param(self, client, clean_state):
        ...
        response = client.get(
            "/api/auth/google/login?redirect=http://127.0.0.1:8080/callback", follow_redirects=False
        )
    ...
    def test_google_callback_creates_jwt(self, mock_client, client, clean_state):
        ...
        oauth_states[state] = {"provider": "google", "redirect": None, "created_at": time.time()}
        ...
        response = client.get(f"/api/auth/google/callback?code=test_code&state={state}")
    ...
    def test_google_callback_invalid_state(self, client, clean_state):
        ...
        response = client.get("/api/auth/google/callback?code=test_code&state=invalid_state")
...
class TestDiscordOAuth:
    ...
    def test_discord_callback_creates_jwt(self, mock_client, client, clean_state):
        ...
        state = secrets.token_urlsafe(16)
        oauth_states[state] = {"provider": "discord", "redirect": None, "created_at": time.time()}
        ...
        response = client.get(f"/api/auth/discord/callback?code=test_code&state={state}")
...
class TestRoleBasedAccessControl:
    ...
    def test_buyer_access_buyer_endpoint(self, protected_client, clean_state):
        ...
        response = protected_client.get("/buyer-only", headers={"Authorization": f"Bearer {token}"})
    ...
    def test_buyer_rejected_from_contributor_endpoint(self, protected_client, clean_state):
        ...
        response = protected_client.get(
            "/contributor-only", headers={"Authorization": f"Bearer {token}"}
        )
...
class TestLogout:
    ...
    def test_logout_with_invalid_token(self, client, clean_state):
        ...
        response = client.post("/api/auth/logout", json={"refresh_token": "nonexistent_token"})
...
class TestLoopbackOAuth:
    ...
    def test_loopback_redirect(self, mock_client, client, clean_state):
        ...
        response = client.get(
            f"/api/auth/google/callback?code=test_code&state={state}", follow_redirects=False
        )
