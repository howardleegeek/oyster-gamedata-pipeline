...
def test_valid_modern_spec_passes(tmp_path):
    p = _write(tmp_path, "Dxx_valid.md", """\
        ---
        task_id: Dxx
        project: oyster
        priority: 1
...
def test_missing_front_matter_passes_with_warning(tmp_path):
    ...
    p = _write(tmp_path, "Dxx_legacy.md", """\
        # Old spec
        Did some real work.
        """)
    res = spec_lint.lint_file(p)
...
def test_modern_spec_missing_required_keys_fails(tmp_path):
    p = _write(tmp_path, "Dxx_bad_fm.md", """\
        ---
        task_id: Dxx
        project: oyster
        ---
...
def test_modern_spec_too_few_acceptance_criteria_fails(tmp_path):
    p = _write(tmp_path, "Dxx_too_few_ac.md", """\
        ---
        task_id: Dxx
        project: oyster
        priority: 1
...
def test_banned_pattern_in_body_fails(tmp_path):
    p = _write(tmp_path, "Dxx_banned.md", """\
        ---
        task_id: Dxx
        project: oyster
        priority: 1
...
def test_banned_pattern_in_donot_section_passes(tmp_path):
    p = _write(tmp_path, "Dxx_donot.md", """\
        ---
        task_id: Dxx
        project: oyster
        priority: 1
...
def test_iron_law_waived_passes(tmp_path):
    p = _write(tmp_path, "Dxx_waived.md", """\
        ---
        task_id: Dxx
        project: oyster
        priority: 1
