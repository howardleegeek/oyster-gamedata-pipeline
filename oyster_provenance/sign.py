...
def verify_signature(
    data: bytes, signature_hex: str, public_key_path: Path = DEFAULT_PUBLIC_KEY_PATH
) -> bool:
    """
...
def verify_json_signature(
    data: dict, signature_hex: str, public_key_path: Path = DEFAULT_PUBLIC_KEY_PATH
) -> bool:
...
def generate_keypair(
    key_dir: Union[str, Path] = DEFAULT_KEY_DIR,
) -> Tuple[SigningKey, PublicKeyInfo]:
    """
    ...
    key_info = PublicKeyInfo(
        ...
        expires_at_utc=time.strftime(
            "%Y-%m-%dT%H:%M:%SZ", time.gmtime(now + 365 * 24 * 60 * 60)
        ),  # 1 year
    )
