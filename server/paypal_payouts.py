...
def get_access_token() -> str:
    ...
    try:
        response = requests.post(
            PAYPAL_OAUTH_URL, auth=auth, data=data, headers={"Accept": "application/json"}
        )
...
def execute_paypal_payout(
    ...
    try:
        response = requests.post(PAYPAL_PAYOUTS_URL, json=payload, headers=_get_headers())
        response.raise_for_status()
...
def list_payouts(contributor_id: Optional[str] = None, limit: int = 10) -> list:
    """List recent payout batches."""
    try:
        params = {"limit": limit}

        response = requests.get(PAYPAL_PAYOUTS_URL, params=params, headers=_get_headers())
        response.raise_for_status()
