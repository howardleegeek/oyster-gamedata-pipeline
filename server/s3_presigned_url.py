...
class MultipartUpload:
    ...
    def get_parts(self) -> List[Dict]:
        ...
        return [
            {"PartNumber": num, "ETag": data["ETag"]} for num, data in sorted(self.parts.items())
        ]
...
def generate_presigned_url(upload_id: str, part_number: int, session_id: str) -> str:
    ...
    url = (
        f"{S3_ENDPOINT}/{S3_BUCKET}/uploads/{session_id}/"
...
def init_multipart():
    ...
    try:
        ...
        logger.info(
            f"Initialized multipart upload: {session_id} ({upload_id}), {total_chunks} chunks"
        )
...
def get_presigned_url():
    ...
    try:
        ...
        return jsonify({"url": url, "expires_at": upload.expires_at, "part_number": part_number})
...
def complete_multipart():
    ...
    try:
        ...
        logger.info(f"Completed multipart upload: {session_id}, {len(upload.parts)} parts")
...
def cleanup_expired_uploads():
    ...
    expired = [
        session_id
        for session_id, upload in multipart_uploads.items()
