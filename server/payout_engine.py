...
async def init_payout(approval: SessionApproval, background_tasks: BackgroundTasks):
    ...
    for existing in payout_queue.values():
        if (
            existing.get("session_id") == approval.session_id
            and existing["status"] == PayoutStatus.COMPLETED
        ):
    ...
    if not check_daily_cap(contributor_id=approval.contributor_id, amount=amount):
        ...
        return {
            "status": "cap_reached",
            "amount": amount,
            "daily_total": calculate_daily_total(approval.contributor_id),
        }
...
async def process_payouts():
    ...
    for payout_id, item in list(payout_queue.items()):
        ...
        try:
            ...
            # Log to audit trail
            _write_audit_log(
                "COMPLETED", payout_id, item["contributor_id"], item["amount"], item["method"]
            )
    ...
    return {"processed": len(processed), "failed": len(failed), "payout_ids": processed}
