...
# Page config
st.set_page_config(
    page_title="Oyster Dashboard", page_icon="🦪", layout="wide", initial_sidebar_state="expanded"
)
...
def render_login():
    ...
    with st.sidebar.form("login_form"):
        ...
        if submit and user_id:
            ...
            try:
                resp = requests.get(
                    f"{API_BASE}/api/auth/token", params={"user_id": user_id, "role": role}
                )
                resp.raise_for_status()
                data = resp.json()
...
def render_browse_page():
    ...
    if sessions:
        ...
        # Format for display
        df_display = df[
            ["id", "game", "scene", "route_type", "audit_score", "status", "duration_seconds"]
        ]
    ...
    if st.button("Verify", type="primary"):
        ...
        if result:
            if result["valid"]:
                ...
                st.markdown(
                    '<span class="verify-valid">Valid Hash Chain</span>', unsafe_allow_html=True
                )
            else:
                st.error("❌ Provenance Verification Failed")
                st.markdown(
                    '<span class="verify-invalid">Invalid Hash Chain</span>', unsafe_allow_html=True
                )
    ...
    for session in sessions:
        with st.container():
            ...
            with col1:
                ...
                st.caption(
                    f"Score: {session['audit_score']:.2f} | Duration: {session['duration_seconds']}s"
                )
        ...
        if st.session_state.get("reject_session") == session["id"]:
            with st.form(f"reject_form_{session['id']}"):
                ...
                if submitted and reason:
                    result = api_post(
                        f"/api/sessions/{session['id']}/reject", {"reason": reason, "notes": notes}
                    )
                    if result:
                        st.success("Rejected!")
    ...
    for session in sessions:
        ...
        with st.container():
            ...
            with col1:
                ...
                st.caption(
                    f"{session['game']} | {session['scene']} | Score: {session['audit_score']:.2f}"
                )
...
def render_payouts_page():
    ...
    if data["payout_history"]:
        ...
        fig.update_layout(xaxis_title="Session", yaxis_title="Amount ($)", height=400)
        st.plotly_chart(fig, use_container_width=True)
