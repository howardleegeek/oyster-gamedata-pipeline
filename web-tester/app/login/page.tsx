...
function LoginInner() {
  ...
  async function handleMagicLink(e: FormEvent) {
    ...
    if (!supabase) {
      setStatus('error');
      setError(
        'Supabase is not configured. In DEV MODE you can browse the app without signing in — just visit /dashboard.'
      );
      return;
    }
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-16">
      ...
      <button onClick={handleGitHub} className="btn-secondary w-full mb-6" disabled={!isSupabaseConfigured()}>
      ...
      {!isSupabaseConfigured() && (
        <p className="mt-6 text-xs text-oyster-400 text-center">
          ...
          <Link href="/dashboard" className="text-amber-accent hover:underline">
            /dashboard
          </Link>{' '}
          directly to see the sample UI.
        </p>
      )}
    </div>
  );
